...
const SmallNavs: React.FC<SmallNavsProps> = ({ nav, apexLogo }) => {
  return (
    <div
      className={
        nav
      ...
      <ul className="w-full mt-10">
        {navLinks.map((link) => (
          <li className="w-full" key={link.href}>
            <Link
              className="bg-zinc-100 hover:bg-zinc-300 flex w-full border border-zinc-200 px-6 py-4 font-semibold"
              href={link.href}
            >
              {link.label}
            </Link>
