...
import { TestimonialType, testimonialData } from '@/components/data/testimonials/testimonialData';
import { delay, motion } from "framer-motion"
import { SocialStagger, fadeInOutLeftToRight, fadeInOutRightToLeft } from "@/components/animation/animation";

export type VariantType = 'chlorelief' | 'lubie';
...
function Testimonials({ variant }: TestimonialsProps) {
  const testimonials = testimonialData.filter((item: TestimonialType) => item.variant === variant);
  const testimonialCards = testimonials.map((item: TestimonialType, index) => {
    return (
      <motion.div
        variants={fadeInOutLeftToRight}
        key={index}
        className="w-full md:w-auto container mx-auto lg:mx-0 justify-center items-center"
      >
        <Cards name={item.name} comment={item.comment} img={item.img} rating={item.rating} />
      </motion.div>
    );
  });

  const containerClassName = `container flex flex-col justify-evenly items-center text-center lg:flex-row lg:text-start gap-4 ${variant === 'chlorelief' ? 'bg-chlorelief-star-green' : 'bg-lubie-navy-blue'}`;
    ...
    >
      ...
      <div className="w-full flex justify-between px-4">
        ...
        <motion.div
          ...
          className="hidden md:flex w-full justify-center gap-4 pt-16 min-[768px]:flex-col lg:flex-row">
          {testimonialCards}
        </motion.div>
      </div>
    </div>
