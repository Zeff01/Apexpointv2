        ...
        {ApexSliderData.map((slide, slide_id) => (
          <SwiperSlide key={slide_id}>
            <Image
              alt="1"
              src={slide.image}
              style={{ backgroundColor: getColor(slide_id, ApexSliderData.length) }}
              className="shadow-lg"
            />
            <Link
              href={slide.productlink}
              className="absolute hover:text-apexpoint-dark-orange  bottom-1  sm:bottom-6 right-6 text-sm md:text-lg text-white"
            >
              View More{' '}
            </Link>
          </SwiperSlide>
